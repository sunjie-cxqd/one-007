...
注：通过left,right,top,bottom中的任意两个属性来确定元素绝对定位的位置
...
eg:  opacity:0.6;   等价于  opacity:.6;

注：低版本的IE浏览器（IE8及以下浏览器）不识别opacity属性，兼容写法如下：

filter:alpha(opacity=数值);   取值范围(0-100)

四、定位元素层叠属性

语法： z-index：auto(默认值)| 整数值；
...
语法： 命名锚记名   <标记  id="锚记名"></标记>
           连接锚记名   <a href="#锚记名"></a>

eg:  <a href="#boy">第一章 html基础</a>
...
注：embed标记主要用来兼容IE浏览器

扩展二：滚动字幕

语法：<marquee behavior="scroll|alternate" width="数值" height="数值" direction="left|right|up|down" scrollamount="数值">滚动内容...</marquee>

注： a) behavior="alternate"   正反方向交替运动
